...
cd '/scratch7/MINDLAB2019_EEG-SubdivisionEntrainment/2021/Pilot01/Pilot1_EEG_MEG'

addpath '/scratch7/MINDLAB2019_EEG-SubdivisionEntrainment/2021/MEEG_data/' %filename b1_ica-raw.fif
...
cfg.channel   = hdr.label(4:78)%{'all'}; %hdr.label(4:78); %,'-CanEye','-LowEye', '-Neck', '-Arm'};  %elimina algun channel {'all','-Veog','-Heog', '-Neck', '-Arm'}

foobar = ft_preprocessing(cfg,dat_2)%(cfg,dat_redef); 

%%
for tt = 1:length(foobar.trial)
    dat_2.trial{tt}(4:78,:) = foobar.trial{tt};
end

%% Delete mastoids
cfg = [];
cfg.channel = hdr.label([4:78 90:395]);%'all'%hdr.label(4:78);%{'all'}% {'all', '-M1', '-M2'}; % delete mastoids
% cfg.outputfile = strcat(subj0,'dat_reref');

dat_reref = ft_preprocessing(cfg,dat_2);
...
% % For endings
% cfg=[]; 
% cfg.channel='allhdr.label(4:78);%'all';
