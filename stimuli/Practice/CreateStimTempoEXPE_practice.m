...
for number=[10, 37, 56, 91 ]%1:length(StimPilot1)%[91 92 94 95 97 98 1:81]
     
        [inwave96, fs] = audioread(['C:\Users\au631438\Desktop\PROJECTS\Groove\SanderGenerator\stims\96-practice\macro-output\' StimNames{number} '.wav']);%' %num2str(number2, '%02.f') '.wav']);%[inwave0, fs] = audioread([StimNames(number) '.wav']);
        %sound(inwave96, fs)
        BarLength=((2.4744)*fs);%round((2.5/4)*fs);
...
plotI=0;
    for  tp=[90 96 104] 
        ...
        addpath(['C:\Users\au631438\Desktop\PROJECTS\Groove\SanderGenerator\stims\' num2str(tp) '-practice\macro-output'])%'C:\Users\au631438\Desktop\PROJECTS\Groove\SoundAmplitudes\'
        ...
        NewStim=[NewStim0; zeros(2, round(2*(2.4744)*fs))'; BarStim.*ramp2']';     
    ...
    inwave=mono;
        ...
        filename=['C:\Users\au631438\Desktop\PROJECTS\Groove\MEG2022\Stimuli\Practice\' num2str(number, '%02.f') '_t' num2str(tp, '%02.f') '.wav'];
        audiowrite(filename,NewStim',fs);
        rmpath(['C:\Users\au631438\Desktop\PROJECTS\Groove\SanderGenerator\stims\' num2str(tp) '-practice\macro-output'])    
    end
    saveas(gcf, ['PlotsTempoStim\PracticeTempoSoundAmp' num2str(number, '%02.f') '.png'])
